...
print("*" * 25)
print("Датасет с аномальными зёрнами")
print("Исследование различных гипотез эволюции бассейна")
print("*" * 25)
...
class ModelParameters:
    ...
    output_dir = "model_output/with_anomalies_search"
    ...
    datafile_output_dir = (
        "model_output/with_anomalies_search/thermal_history_datafiles"
    )
    ...
    exhumation_period_starts = np.array([120], dtype=float)
    # end of exhumation phase (Ma)
    exhumation_period_ends = np.array([70], dtype=float)

    # thickness eroded in each exhumation phase (m)
    exhumed_thicknesses = np.array([1000], dtype=float)
    ...
    # the model will add units starting from the oldest to the youngest, until the additional thickness needed for
    # erosion is filled
    exhumed_strat_units = [["ATBRM"]]

    # original thicknesses of the removed units (aligned with exhumed_strat_units)
    original_thicknesses = [[1200]]

    # -------- Heatflow history --------
    heatflow_ages = np.array([300, 80, 0], dtype=float)  # Ma (past -> present)
    heatflow_history = np.array(
        [0.09075, 0.0825, 0.0825], dtype=float
    )  # W/m^2
    ...
    # original_thicknesses = [[1200.0]]
    # original_thicknesses = [[200, 200, 200, 200, 200]]
