...
def model_data_comparison_T(T_data_well, z_nodes, T_nodes, active_nodes):
    ...
    T_data_well["simulated_T"] = np.interp(
        T_data_well["depth"],
    ...
    T_data_well["residual"] = (
        T_data_well["temperature"] - T_data_well["simulated_T"]
    )
    T_data_well["residual_norm"] = (
        T_data_well["residual"] / T_data_well["temperature_unc_1sigma"]
    )
    T_data_well["P_fit"] = (
        1.0 - scipy.stats.norm.cdf(np.abs(T_data_well["residual_norm"]))
    ) * 2
...
def model_data_comparison_VR(
    vr_data_well, z_nodes, vr_nodes, active_nodes, vr_unc_sigma=0.05
):
    """
    ...
    vr_data_well["residual"] = (
        vr_data_well["VR"] - vr_data_well["simulated_vr"]
    )
    ...
    vr_data_well["residual_norm"] = (
        vr_data_well["residual"] / vr_data_well["VR_unc_1sigma"]
    )
    vr_data_well["P_fit"] = (
        1.0 - scipy.stats.norm.cdf(np.abs(vr_data_well["residual_norm"]))
    ) * 2
...
def model_data_comparison_AFT_age(
    aft_data_well,
    aft_ages,
    ...
    for sample in aft_data_well["sample"]:
        ...
        if sample in aft_ages["sample"].values:
            ...
            single_grain_ages_se_min_sample = aft_ages["aft_age_stderr_min"][
                ind_sample
            ].values
            single_grain_ages_se_plus_sample = aft_ages["aft_age_stderr_plus"][
                ind_sample
            ].values
            ...
            age_bin, age_pdf = pybasin_lib.calculate_aft_ages_pdf(
            ...
            if (
                aft_data_well.loc[ind_sample, "aft_age"].values == 0
                and aft_data_well.loc[ind_sample, "aft_age_stderr_plus"].values
                == 0
            ):
    ...
    for i, sample_ix, age_bin, age_pdf in zip(
        itertools.count(), aft_data_well.index, age_bins, age_pdfs
    ):
        ...
        if np.any(np.isnan(age_pdf)) == False:
            ...
            end_ind = np.where(
                aft_data_well.loc[sample_ix, "simulated_AFT_max"] < age_bin
            )[0][0]

            pdf_fit_sum = np.sum(age_pdf[start_ind:end_ind])
            pdf_nofit_sum = np.sum(age_pdf[:start_ind]) + np.sum(
                age_pdf[end_ind:]
            )
    ...
    for i, sample_ix, age_bin, age_pdf in zip(
        ...
        if np.any(np.isnan(age_pdf)) == False:
            ...
            print(
                "no model-data comparison for sample %s, "
...
def model_data_comparison_he(
    ...
    for he_sample_i, he_sample_ix, he_sample in zip(
        itertools.count(), he_samples_well.index, he_samples_well["sample"]
    ):
        ...
        if he_sample in he_data["sample"].values:
            ...
            for grain_i, he_age_obs, he_age_obs_SE in zip(
                itertools.count(),
                he_data.loc[ind_sample, "he_age_uncorrected"].values,
                he_data.loc[ind_sample, "he_age_uncorrected_se"].values,
            ):

                he_age_pdf = scipy.stats.norm.pdf(
                    he_age_bin, he_age_obs, he_age_obs_SE
                )
                ...
                he_age_sim_min = modeled_he_age_samples_min[he_sample_i][
                    grain_i
                ]
                he_age_sim_max = modeled_he_age_samples_max[he_sample_i][
                    grain_i
                ]
            ...
            he_samples_well.loc[he_sample_ix, "mean_GOF_all_grains"] = np.mean(
                np.array(grain_pdfs)
            )
            he_samples_well.loc[he_sample_ix, "min_GOF_all_grains"] = np.min(
                np.array(grain_pdfs)
            )
            he_samples_well.loc[he_sample_ix, "max_GOF_all_grains"] = np.max(
                np.array(grain_pdfs)
            )
            he_samples_well.loc[he_sample_ix, "mean_he_error"] = (
                age_error / len(grain_pdfs)
            )
    ...
    if "mean_he_error" in he_samples_well.columns:
        he_age_error = he_samples_well.loc[
            he_sample_ix, "mean_he_error"
        ].mean()
    else:
...
def model_data_comparison_salinity(
    ...
    salinity_data_well["simulated_salinity"] = np.interp(
    ...
    salinity_data_well["P_fit"] = (
        1.0 - scipy.stats.norm.cdf(np.abs(salinity_data_well["residual_norm"]))
    ) * 2
...
def assemble_data_and_simulate_aft(
    resample_t,
    nt_prov,
    n_nodes,
    time_array_bp,
    ...
    if calculate_thermochron_for_all_nodes is True:
        ...
        simulated_AFT_data = pybasin_lib.simulate_aft(
            resample_t,
            nt_prov,
            n_nodes,
            time_array_bp,
    ...
    for h in range(n_prov):
        prov_start_samples[:, h] = np.interp(
            aft_data_well["depth"],
            z_nodes[-1, active_nodes[-1]],
            prov_start_nodes[active_nodes[-1], h],
        )
        prov_end_samples[:, h] = np.interp(
            aft_data_well["depth"],
            z_nodes[-1, active_nodes[-1]],
    ...
    for h in range(nt):
        active_nodes_aft_samples[h, :] = np.interp(
            aft_data_well["depth"],
            z_nodes[-1, active_nodes[-1]],
    ...
    simulated_aft_data_samples = pybasin_lib.simulate_aft(
        resample_t,
        nt_prov,
        n_aft_samples,
        time_array_bp,
...
def assemble_data_and_simulate_he(
    he_samples_well,
    he_data,
    decay_constant_238U,
    decay_constant_235U,
    ...
    if calculate_thermochron_for_all_nodes is True:
        ...
        for he_sample_no, he_sample in enumerate(samples):
            ...
            if True in ind_sample.values:
                if (
                    ...
                    he_grain_radius_nodes[:, 0] = np.min(
                        he_grain_radius_sample
                    )
                if (
                    ...
                    he_grain_radius_nodes[:, 1] = np.max(
                        he_grain_radius_sample
                    )
        ...
        simulated_He_data = pybasin_lib.simulate_ahe(
            resample_t,
            nt_prov,
            n_nodes,
            time_array_bp,
            ...
            provenance_start_temp=provenance_start_temp,
            log_tT_paths=False,
            tT_path_filename=tT_path_filename,
        )
    ...
    if n_he_samples == 0:
        return (None, None, None, None, None, simulated_He_data)

    # get T history for samples only
    T_he_samples = np.zeros((nt, n_he_samples))
    for h in range(nt):
        T_he_samples[h, :] = np.interp(
            he_samples_well["depth"],
            z_nodes[-1, active_nodes[-1]],
            T_nodes[h, active_nodes[-1]],
        )
    ...
    for h in range(n_prov):
        prov_start_he_samples[:, h] = np.interp(
            he_samples_well["depth"],
            z_nodes[-1, active_nodes[-1]],
            prov_start_nodes[active_nodes[-1], h],
        )
        prov_end_he_samples[:, h] = np.interp(
            he_samples_well["depth"],
            z_nodes[-1, active_nodes[-1]],
            prov_end_nodes[active_nodes[-1], h],
        )
    ...
    # get active node array for samples only
    active_nodes_he_samples = np.zeros((nt, n_he_samples), dtype=bool)
    for h in range(nt):
        active_nodes_he_samples[h, :] = np.interp(
            he_samples_well["depth"],
            z_nodes[-1, active_nodes[-1]],
            active_nodes[h, active_nodes[-1]],
        )
    ...
    simulated_he_data_samples = pybasin_lib.simulate_ahe(
        resample_t,
        nt_prov,
        n_he_samples,
        time_array_bp,
        z_he_samples,
        T_he_samples,
        active_nodes_he_samples,
        prov_start_he_samples,
        prov_end_he_samples,
        surface_temp,
        minerals,
        he_grain_radius_samples,
...
def run_model_and_compare_to_data(
    well_number,
    well,
    well_strat,
    strat_info_mod,
    ...
    model_result_vars = pybasin_lib.run_burial_hist_model(
        well_number,
        well,
        well_strat,
        strat_info_mod,
    ...
    for i, exhumed_unit_block in enumerate(exhumed_unit_blocks):
        ...
        if start * 1e6 in time_array_bp and end * 1e6 in time_array_bp:
            ...
            print(
                "could not find exact start and end of "
        ...
        model_results_series["mean_cooling_exhumation_phase_%i" % i] = (
            T_cooling_preserved.mean()
        )
        model_results_series["min_cooling_exhumation_phase_%i" % i] = (
            T_cooling_preserved.min()
        )
        model_results_series["max_cooling_exhumation_phase_%i" % i] = (
            T_cooling_preserved.max()
        )

    # record max temperature and depth
    model_results_series["max_temperature"] = T_nodes.max()
    model_results_series["max_present_temperature"] = T_nodes[
        -1, active_nodes[-1]
    ].max()
    ...
    if pybasin_params.simulate_VR is True:
        ...
        if (
            ...
            model_results_series["vr_surface"] = vr_nodes[
                -1, active_nodes[-1]
            ][0]
            model_results_series["vr_bottom"] = vr_nodes[-1, active_nodes[-1]][
                -1
            ]
    ...
    if pybasin_params.simulate_salinity is True:
        ...
        depth_to_1g_per_l = np.interp(
            [1.0], C_nodes_gl, z_nodes[-1, active_nodes[-1]]
        )
        depth_to_seawater_salinity = np.interp(
        ...
        model_results_series["depth_to_C=0.035kg/kg"] = (
            depth_to_seawater_salinity
        )

    if (
        pybasin_params.simulate_AFT is True
    ...
    if pybasin_params.simulate_AFT is True:
        ...
        (
            ...
            z_aft_samples,
            T_samples,
        ) = assemble_data_and_simulate_aft(
        ...
        if simulated_AFT_data is not None:
            ...
            model_results_series["aft_age_surface_min"] = aft_age_nodes[
                active_nodes[-1]
            ][0].min()
            model_results_series["aft_age_surface_max"] = aft_age_nodes[
                active_nodes[-1]
            ][0].max()
            model_results_series["aft_age_bottom_min"] = aft_age_nodes[
                active_nodes[-1]
            ][-1].min()
            model_results_series["aft_age_bottom_max"] = aft_age_nodes[
                active_nodes[-1]
            ][-1].max()
            ...
            if aft_age_nodes_min.min() < full_resetting_age:
                ...
                model_results_series["full_resetting_depth_aft_model_min"] = (
                    z_nodes[-1][ind_min][0]
                )
                model_results_series["full_resetting_depth_aft_model_min"] = (
                    T_nodes[-1][ind_min][0]
            ...
            if aft_age_nodes_max.min() < full_resetting_age:
                ...
                model_results_series["full_resetting_depth_aft_model_max"] = (
                    z_nodes[-1][ind_max][0]
                )
                model_results_series["full_resetting_T_aft_model_min"] = (
                    T_nodes[-1][ind_max][0]
                )
            else:
                model_results_series["full_resetting_depth_aft_model_max"] = (
            ...
            if (
                calculate_resetting_depth is True
                ...
                if True in ind_reset_min:
                    model_results_series[
                        "partial_resetting_depth_aft_model_min"
                ...
                if True in ind_reset_max:
                    model_results_series[
                        "partial_resetting_depth_aft_model_max"
    ...
    if pybasin_params.simulate_He is True:
        ...
        (
            ...
            provenance_start_temp=pybasin_params.provenance_start_temp,
            log_tT_paths=pybasin_params.log_tT_paths,
            tT_path_filename=pybasin_params.datafile_output_dir,
        )
        ...
        if simulated_He_data is not None:
            ...
            model_results_series["he_age_surface_min"] = he_age_nodes_array[
                active_nodes[-1]
            ][0].min()
            model_results_series["he_age_surface_max"] = he_age_nodes_array[
                active_nodes[-1]
            ][0].max()
            model_results_series["he_age_bottom_min"] = he_age_nodes_array[
                active_nodes[-1]
            ][-1].min()
            model_results_series["he_age_bottom_max"] = he_age_nodes_array[
                active_nodes[-1]
            ][-1].max()
            ...
            if he_age_nodes_min_array.min() < full_resetting_age:
                ...
                model_results_series["full_resetting_depth_he_model_min"] = (
                    z_nodes[-1][ind_min][0]
                )
                model_results_series["full_resetting_T_he_model_min"] = (
                    T_nodes[-1][ind_min][0]
                )
            else:
                model_results_series["full_resetting_depth_he_model_min"] = (
            ...
            if he_age_nodes_max_array.min() < full_resetting_age:
                ...
                model_results_series["full_resetting_depth_he_model_max"] = (
                    z_nodes[-1][ind_max][0]
                )
                model_results_series["full_resetting_T_he_model_max"] = (
                    T_nodes[-1][ind_max][0]
                )
            else:
                model_results_series["full_resetting_depth_he_model_max"] = (
            ...
            if (
                ...
                if True in ind_reset_min:
                    model_results_series[
                        "partial_resetting_depth_he_model_min"
                    ] = z_nodes[-1][ind_reset_min].min()
                else:
                    model_results_series[
                        "partial_resetting_depth_he_model_min"
                ...
                if True in ind_reset_max:
                    model_results_series[
                        "partial_resetting_depth_he_model_max"
                    ] = z_nodes[-1][ind_reset_max].min()
                else:
                    model_results_series[
                        "partial_resetting_depth_he_model_max"
    ...
    if True in ind.values:
        ...
        T_gof, T_rmse = model_data_comparison_T(
            T_data_well, z_nodes, T_nodes, active_nodes
        )
    ...
    if pybasin_params.simulate_He is True:
        ...
        # calculate model error fission track data
        ind = (he_samples["location"] == well) & (
            he_samples["depth"] <= z_nodes[-1].max() + 1.0
        )
        he_samples_well = he_samples[ind]
    ...
    if pybasin_params.simulate_He is True and location_has_He_data is True:
        He_model_data = [
            he_samples_well["depth"].values,
            he_ages_all_samples,
            he_ages_all_samples_SE,
            he_age_bin,
            he_age_pdfs_all_samples,
...
def update_model_params_and_run_model_new(
    model_scenario_number,
    pybasin_params,
    param_names,
    param_set,
    ...
    if log_screen_output is True:
        ...
        log_fn = "log_well_%s_model_scen_%i_PID_%s.out" % (
            well,
            model_scenario_number,
            str(os.getpid()),
        )
        ...
        err_fn = "error_log_well_%s_model_scen_%i_PID_%s.out" % (
            well,
            model_scenario_number,
            str(os.getpid()),
        )
        ...
        print(
            "redirecting screen output for model errors to error log file %s"
            % err_path
        )
    ...
    for scenario_param_name, scenario_parameter in zip(param_names, param_set):
        ...
        if scenario_parameter is not None:
            ...
            if hasattr(pybasin_params, model_param_name) is False:
                msg = (
                    "error, the parameter %s is not in the ModelParameters class in the pybasin_params.py file"
                    % model_param_name
                )
            ...
            print(
                "updating parameter %s from %s to %s"
                % (
                    model_param_name,
    ...
    pybasin_params.exhumation_period_starts = np.array(
        pybasin_params.exhumation_period_starts
    )
    pybasin_params.exhumation_period_ends = np.array(
    ...
    if hasattr(pybasin_params, "exhumation_durations"):
        print(
            "using exhumation duration and using this to calculate end of exhumation phase"
        )
        pybasin_params.exhuamtion_durations = np.array(
        ...
        print(
            "calculated end of exhumation period: ",
            pybasin_params.exhumation_period_ends,
        )
    ...
    if hasattr(pybasin_params, "provenance_start_temp") is False:
        print(
            "no provenance_start_temp specified in input file, using a value of 120 degrees C"
        )
    ...
    if record_data is True:
        ...
        if pybasin_params.simulate_salinity is True:
            ...
            model_results_series["cumalative_solute_flux_top_kg_m-2"] = np.sum(
                total_solute_flux_top
            )
            model_results_series["cumalative_solute_flux_bottom_kg_m-2"] = (
...
def check_input_data_files(input_dir, pybasin_params):
    ...
    for fn in fns:
        if os.path.exists(os.path.join(input_dir, fn)) is False:
            msg = (
                "error, could not find input file %s in input directory %s"
                % (fn, input_dir)
            )
            raise IndexError(msg)
...
def read_model_input_data(input_dir, pybasin_params):
    ...
    strat_info = pd.read_csv(
        os.path.join(input_dir, "stratigraphy_info.csv"), skip_blank_lines=True
    )
    ...
    # well stratigraphy
    well_strats = pd.read_csv(
        os.path.join(input_dir, "well_stratigraphy.csv"), skip_blank_lines=True
    )

    # surface temperature history
    surface_temp = pd.read_csv(
        os.path.join(input_dir, "surface_temperature.csv"),
    ...
    if pybasin_params.simulate_salinity is True:
        ...
        df_sal_inp = pd.read_csv(
            os.path.join(input_dir, "surface_salinity.csv"),
            skip_blank_lines=True,
        )
    ...
    litho_props = pd.read_csv(
        os.path.join(input_dir, "lithology_properties.csv"),
        skip_blank_lines=True,
    )
    litho_props = litho_props.set_index("lithology")

    # present-day temperature data
    T_data_df = pd.read_csv(
        os.path.join(input_dir, "temperature_data.csv"), skip_blank_lines=True
    )

    if pybasin_params.simulate_VR is True:
        # vitrinite reflectance data
        vr_data_df = pd.read_csv(
            os.path.join(input_dir, "vitrinite_reflectance.csv"),
            skip_blank_lines=True,
        )
    else:
        vr_data_df = None
    ...
    # fission track data
    if pybasin_params.simulate_AFT is True:
        aft_samples = pd.read_csv(
            os.path.join(input_dir, "aft_samples.csv"), skip_blank_lines=True
        )

        # load AFT age data
        aft_ages = pd.read_csv(
            os.path.join(input_dir, "aft_data.csv"), skip_blank_lines=True
        )
    else:
        aft_samples = None
        aft_ages = None
    ...
    if pybasin_params.simulate_salinity is True:
        ...
        salinity_data = pd.read_csv(
            os.path.join(input_dir, "salinity_data.csv"), skip_blank_lines=True
        )
    ...
    litho_cols = [
        ...
        if "provenance" not in col and "marine" not in col
    ]
    ...
    try:
        ...
        print(
            "not all lithology units found in strat info file are also in the "
    ...
    for p in prov_cols:
        if np.all(strat_info[p].isnull()):
            msg = (
                "error in parsing stratigraphy_info.csv file, "
    ...
    if (
        well_strats["depth_top"].min() < 0
        or well_strats["depth_bottom"].min() < 0
    ):

        msg = (
            "error, found a negative value for depth in the depth_top or "
    ...
    for litho_prop_name in litho_props.columns:
        ...
        for col in litho_cols:
            ...
            s = (
                strat_info[col].astype(float)
                * litho_props[litho_prop_name][col]
            )

            # add fraction to column in strat dataframe
            strat_info_mod[litho_prop_name] = (
                strat_info_mod[litho_prop_name] + s
            )
...
def select_well_salinity_bnd(well, salinity_bnd_df):
    ...
    if well in salinity_bnd_df.columns:
        ...
        surface_salinity_well["surface_salinity"] = surface_salinity_well[well]
    else:
...
def setup_model_scenarios_new(ParameterRanges):
    ...
    scenario_param_names = [
        m for m in scenario_param_names_raw if "__" not in m and "_s" in m
    ]

    scenario_parameter_list = [getattr(pr, p) for p in scenario_param_names]

    # construct list with all parameter combinations
    if pr.parameter_combinations is True:
        scenario_parameter_combinations = list(
            itertools.product(*scenario_parameter_list)
        )
    else:
        nscens = np.sum(
            np.array(
                [len(sp) for sp in scenario_parameter_list if sp is not None]
            )
        )
...
def setup_model_output_df(n_scenarios):
    ...
    model_results_df = pd.DataFrame(index=model_scenario_numbers, columns=cols)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PyBasin: Model burial, exhumation and thermal history "
        "and low-temperature thermochronology"
    )

    parser.add_argument(
        "model_input_subfolder",
        metavar="directory",
        default=None,
        nargs="?",
    ...
    if args.model_input_subfolder is not None:
        model_input_subfolder = os.path.join(
            scriptdir, args.model_input_subfolder
        )
    else:
    ...
    if (
        Parameters.save_model_run_data is True
        and os.path.exists(datafile_output_dir) is False
    ):
        print(
            "creating directory %s to store model result datafiles"
            % datafile_output_dir
        )
    ...
    if os.path.exists(fig_output_dir) is False:
        print(
            "creating directory %s to store model-data comparison figures"
            % fig_output_dir
        )
    ...
    model_scenario_param_names, model_scenario_param_list = (
        setup_model_scenarios_new(ParameterRanges)
    )
    ...
    if len(model_scenario_param_names) == 0:
        ...
        print(
            "single model run, setting up parameter set with base case values"
        )
    ...
    if ParameterRanges.parallel_model_runs is True:
        ...
        print(
            "initialized parallel model runs with max %i simultaneous processes"
    ...
    for well_number, well in enumerate(wells):
        ...
        print("x" * 20)
        print("well %s, %i/%i" % (well, well_number + 1, len(wells)))

        if np.any(well_strats["well"] == well) == False:
            raise IOError(
                "error, could not find well %s in well strat file in directory %s"
                % (well, input_dir)
            )
        ...
        if (
            Parameters.simulate_salinity is True
            and Parameters.well_specific_surface_salinity_bnd is True
        ):

            surface_salinity_well = select_well_salinity_bnd(
        ...
        well_strat["thickness"] = (
            well_strat["depth_bottom"] - well_strat["depth_top"]
        )
        well_strat["n_nodes_est"] = np.ceil(
            well_strat["thickness"] / Parameters.max_thickness
        )
        # buffer of 20000 m for number of nodes to be safe
        n_nodes_est = int(
            np.sum(well_strat["n_nodes_est"]) + 2e4 / Parameters.max_thickness
        )
        ...
        if Parameters.simulate_salinity is True:
            n_ts_est = int(
                strat_info_mod["age_bottom"].max()
                * 1e6
                / Parameters.max_hf_timestep
                * 3
            )
        ...
        for well_scenario_no, model_scenario_params in enumerate(
            model_scenario_param_list
        ):

            print("-" * 20)
            print(
                "setting up model scenario %i / %i"
                % (model_scenario_number + 1, len(model_scenario_param_list))
            )
            print("-" * 20)
            ...
            if (
                model_scenario_number / 250 == model_scenario_number / 250.0
                ...
                time_left = (
                    n_scenarios - model_scenario_number
                ) * time_per_scenario
            ...
            if ParameterRanges.parallel_model_runs is False:
                ...
                (
                    ...
                    he_age_error,
                    He_model_data,
                    model_results_series_updated,
                ) = update_model_params_and_run_model_new(
                    model_scenario_number,
                    Parameters,
                    model_scenario_param_names,
                ...
                model_results_df.loc[model_scenario_number_store] = (
                    model_results_series_updated
                )
            ...
            while keep_on_processing is True:
                ...
                if ParameterRanges.parallel_model_runs is False:
                    ...
                    for ip, p in enumerate(processes):
                        ...
                        if (
                            model_result_ready is False
                            and p.ready() is True
                            and done_processing[ip] is False
                        ):
                ...
                if model_result_ready is False:
                    ...
                    if (
                        save_counter == 0
                        or save_counter >= save_counter_interval
                    ):
                        ...
                        fn = os.path.join(
                            output_dir,
                            "model_results_%s_%s_ms0-%i.csv"
                            % (today_str, well_txt, n_scenarios),
                        )
                        print("saving model results .csv file %s" % fn)
                        model_results_df.to_csv(
                            fn, index_label="model_scenario_number"
                        )
                    ...
                    if Parameters.save_model_run_data is True:
                        ...
                        df_tt2 = pd.DataFrame(
                            columns=cols, index=np.arange(n_steps)
                        )
                        ...
                        if Parameters.simulate_salinity is True:
                            ...
                            dfc.loc[
                                :l,
                                "salinity_s%i" % model_scenario_number_store,
                            ] = C_nodes[-1, active_nodes[-1]]

                            # save solute flux data
                            columns = [
                                "solute_flux_top_kg_m-2_s-1",
                            ...
                            dfqs.loc[
                                :lqs,
                                "solute_flux_top_kg_m-2_s-1_s%i"
                                % model_scenario_number_store,
                            ] = q_solute_top
                            dfqs.loc[
                                :lqs,
                                "solute_flux_bottom_kg_m-2_s-1_s%i"
                                % model_scenario_number_store,
                            ] = q_solute_bottom
                            dfqs.loc[
                                :lqs,
                                "time_yr_s%i" % model_scenario_number_store,
                            ] = time_array_bp
                            fn = os.path.join(
                                csv_output_dir,
                        ...
                        if Parameters.simulate_VR is True:
                            ...
                            if vr_nodes is not None:
                                ...
                                dfc.loc[
                                    ind_nn,
                                    "depth_s%i" % model_scenario_number_store,
                                ] = z_nodes[-1, active_nodes[-1]]
                                dfc.loc[
                                    ind_nn,
                                    "T_s%i" % model_scenario_number_store,
                                ] = T_nodes[-1, active_nodes[-1]]
                                dfc.loc[
                                    ind_nn,
                                    "VR_s%i" % model_scenario_number_store,
                                ] = vr_nodes[-1, active_nodes[-1]]
                                ...
                                print(
                                    "saving depth, temperature and VR data to %s"
                                    % fn
                                )
                                ...
                                fn = os.path.join(
                                    csv_output_dir,
                                    "model_data_comparison_VR_%s_%s_ms%i.csv"
                                    % (
                                        well_store,
                                        today_str,
                                        model_scenario_number_store,
                        ...
                        # AFT data:
                        if (
                            Parameters.simulate_AFT is True
                            and AFT_data is not None
                        ):
                            ...
                            for sample_i in range(n_samples):
                                for model_j in range(n_models):
                                    cols = cols + [
                                        "name_sample_%i_model_%i"
                                        % (sample_i, model_j)
                                    ]
                                    cols = cols + [
                                        "depth_sample_%i_model_%i"
                                        % (sample_i, model_j)
                                    ]
                                    cols = cols + [
                                        "aft_age_sample_%i_model_%i"
                                        % (sample_i, model_j)
                                    ]
                                    cols = cols + [
                                        "time_sample_%i_model_%i"
                                        % (sample_i, model_j)
                                    ]
                                    cols = cols + [
                                        "temp_sample_%i_model_%i"
                                        % (sample_i, model_j)
                                    ]

                            df_tt_aft = pd.DataFrame(
                                columns=cols, index=np.arange(max_steps)
                            )
                            ...
                            print(
                                "saving time-temperature paths AFT samples to %s"
                                % fn
                            )
                            ...
                            print(
                                "saving modeled AFT data for samples to %s"
                                % fn
                            )
                            # df_aft.to_csv(fn)
                        ...
                        #  U-Th/He data
                        if (
                            Parameters.simulate_He is True
                            and He_model_data is not None
                        ):
                            ...
                            for sample_i, age_mod_sample in enumerate(
                                modeled_he_age_samples
                            ):
                                for grain_i, age_mod_grains in enumerate(
                                    age_mod_sample
                                ):
                                    for (
                                        prov_model_i,
                                        age_mod_prov,
                                    ) in enumerate(age_mod_grains):
                                        he_data_samples.loc[
                                            sample_i,
                                            "modeled_he_age_grain_%i_prov_%i"
                                            % (grain_i, prov_model_i),
                                        ] = age_mod_prov
                                fn = os.path.join(
                                    csv_output_dir,
                                ...
                                print(
                                    "saving modeled  U-Th/He data to %s" % fn
                                )
                    ...
                    if Parameters.make_model_data_fig is True:
                        ...
                        if type(Parameters.fig_adj) is list:
                            for fa in Parameters.fig_adj:
                                ...
                                print(
                                    "saving model-data comparison figure %s"
                                    % fn
                                )
                                fig.savefig(fn, dpi=200)
                            ...
                            print(
                                "saving model-data comparison figure %s" % fn
                            )
                            fig.savefig(fn, dpi=200)
                            pl.clf()
                ...
                if ParameterRanges.parallel_model_runs is True:
                    ...
                    last_model_scenario = model_scenario_number >= (
                        n_scenarios - 1
                    )
                    if (
                        n_open_processes
                        >= ParameterRanges.max_number_of_processes
                        or last_model_scenario is True
                    ):
                        keep_on_processing = True
                    else:
        ...
        for col in cols_to_separate:
            ...
            try:
                new_cols = [
                    ast.literal_eval(",".join(c.split()))
                    for c in model_results_df[col].values.tolist()
                ]
                n_new_cols = len(new_cols[0])
                new_cols_a = np.array(new_cols)

                new_col_names = [col + "_" + str(i) for i in range(n_new_cols)]
                for new_col_name, new_col_value in zip(
                    new_col_names, new_cols_a.T
                ):
                    model_results_df[new_col_name] = new_col_value
            except:
                print(
                    "failed to separate column %s in model output dataframe / csv file"
                    % col
                )
        ...
        fn = os.path.join(
            output_dir,
            "model_results_%s_%s_ms0-%i_final.csv"
            % (today_str, well_txt, n_scenarios),
        )
        print("saving model results .csv file %s" % fn)
