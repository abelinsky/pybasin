...
def compact(bm, n0, c, z_top, b_guess, max_decompaction_error, verbose=False):
    """ """
    ...
    while thickness_diff_max > max_decompaction_error:
        ...
        if verbose is True:
            print(
                "max change in calculating compacted thickness = %0.2f"
                % thickness_diff_max
            )
...
def subdivide_strat_units(input_df, max_thickness):
    ...
    if np.any(ind_ths.values) == True:
        ...
        for ind_th in input_df.index[ind_ths]:
            ...
            n_subdiv = int(
                np.ceil(
                    input_df.loc[ind_th, "present-day_thickness"]
                    / max_thickness
                )
            )
            new_th = input_df.loc[ind_th, "present-day_thickness"] / n_subdiv
            new_duration = (
                input_df.loc[ind_th, "age_top"]
                - input_df.loc[ind_th, "age_bottom"]
            ) / n_subdiv
            ...
            input_df.loc[ind_th, "age_top"] = (
                input_df.loc[ind_th, "age_bottom"] + new_duration
            )
            input_df.loc[ind_th, "depth_top"] = (
                input_df.loc[ind_th, "depth_bottom"] - new_th
            )
        ...
        # subdiv_df = pd.DataFrame(
        #     index=subdiv_units, columns=input_df.columns, dtype="float64"
        # )
        subdiv_df = pd.DataFrame(
            index=subdiv_units, columns=input_df.columns, dtype=object
        )
        ...
        for i, unit in enumerate(subdiv_units):
            ...
            try:
                # print(subdiv_df.loc[unit, input_df.columns])
                # print("==============")
                # print(input_df.loc[orig_unit, input_df.columns])

                subdiv_df.loc[unit, input_df.columns] = input_df.loc[
                    orig_unit, input_df.columns
                ]
...
def add_exhumation_phases(
    ...
    if exhumation_active is False:
        print(
            "no exhumation phase found that is younger than oldest strat. "
            "unit for this well"
        )
        return well_strat
    ...
    for (
        ...
        if exhumed_thickness > 0 and exhumation_period_start < oldest_unit:
            ...
            try:
                ...
                print(
                    "error, original thicknesses list is longer or "
            ...
            for unit in df_ex.index:
                ...
                df_ex.loc[unit, "preserved_thickness"] = well_strat[
            ...
            try:
                ...
                msg = (
                    "error, cannot implement exhumation. most likely the youngest preserved stratigraphic unit is "
            ...
            eroded_units = exhumed_strat_unit[
                exhumed_strat_unit.index(youngest_unit) :
            ]

            # calculate eroded thicknesses
            # Преобразуем preserved_thickness к числовому типу
            df_ex["preserved_thickness"] = pd.to_numeric(
                df_ex["preserved_thickness"], errors="coerce"
            )

            print(df_ex.loc[eroded_units, "eroded_thickness"])
            print("======")
            print(df_ex.loc[eroded_units, "normal_thickness"])
            print("======")
            print(df_ex.loc[eroded_units, "preserved_thickness"])
            print("======")
            ...
            if df_ex["eroded_thickness"].sum() > exhumed_thickness:
                ...
                for eroded_unit in eroded_units[::-1]:
                    if ex_diff > 0:
                        if (
                            df_ex.loc[eroded_unit, "eroded_thickness"]
                            > ex_diff
                        ):
                            df_ex.loc[
                                eroded_unit, "eroded_thickness"
                            ] -= ex_diff
                            ex_diff = 0
                        else:
                            ex_diff -= df_ex.loc[
            ...
            df_ex["n_additional_units"] = np.ceil(
                (df_ex["eroded_thickness"] / max_thickness).astype(float)
            )

            df_ex.loc[
                df_ex["n_additional_units"] == 0, "n_additional_units"
            ] = np.nan
            ...
            df_ex.loc[eroded_units, "age_bottom"] = strat_info_mod.loc[
                eroded_units, "age_bottom"
            ]
            df_ex.loc[eroded_units, "age_top"] = strat_info_mod.loc[
                eroded_units, "age_top"
            ]
            ...
            youngest_unit_index = [
                i for i, wsli in enumerate(wsl) if youngest_unit in wsli
            ][0]
            ...
            if (
                ...
                exhumation_period_end = well_strat.loc[
                    overlying_unit, "age_bottom"
                ]
            ...
            exhumation_duration_total = (
                exhumation_period_start - exhumation_period_end
            )
            ...
            if df_ex.loc[youngest_unit, "duration_non_preserved_units"] > 0:
                ...
                durations_fr = (
                    -np.arange(n_youngest_units + 1).astype(float)
                    / n_youngest_units
                )
                starts_y = (
                    durations_fr[:-1]
            ...
            for eroded_unit in eroded_units:
                ...
                if df_ex.loc[eroded_unit, "n_additional_units"] > 0:
                    ...
                    start = (
                        df_ex.loc[eroded_unit, "age_bottom"]
                    ...
                    new_units_start = (
                        ...
                        / df_ex.loc[eroded_unit, "n_additional_units"]
                        * df_ex.loc[
                            eroded_unit, "duration_non_preserved_units"
                        ]
                        + start
                    )
                    new_units_end = (
                    ...
                    if True in ind:
                        print(
                            "warning, negative value in exhumation timing",
                            ex_units_end[ind],
                        )
                    ...
                    if True in ind:
                        print(
                            "warning, negative duration of exhumation timestep",
                            ex_units_start[ind],
                            ex_units_end[ind],
                        )
                        print("setting duration to %0.2e" % min_exh_duration)
                        ex_units_end[ind] = (
                            ex_units_start[ind] - min_exh_duration
                        )
    ...
    if two_stage_exh is True:
        ...
        new_units_start_mod = np.array(new_units_start_list, dtype=float)[
            ind_exh
        ]
        new_units_end_mod = np.array(new_units_end_list, dtype=float)[ind_exh]
        duration_exh = (
            np.array(new_units_start_list, dtype=float)[ind_exh]
        ...
        end_exhumation_segment1 = int(
            np.round(len(duration_exh) * exhumation_segment_factor)
        )
        duration_exh_new = duration_exh.copy()

        # adjust duration of first exhumation segment
        a = (
            np.sum(duration_exh_new[:end_exhumation_segment1])
            / duration_exh_total
        )
        duration_exh_new[:end_exhumation_segment1] *= (
            exhumation_duration_factor / a
        )

        # change number and time of exhumation 2nd segment
        # ne_left = len(duration_exh_new[end_exhumation_segment1:])
        time_left = duration_exh_total - np.sum(
            duration_exh_new[:end_exhumation_segment1]
        )
        ...
        if np.abs(np.sum(duration_exh_new) - duration_exh_total) > 0.0001:
            msg = (
                "error, distributing exhumation in 2 segments resulted "
        ...
        for i in range(1, len(new_units_start_mod) + 1):
            ...
            new_units_end_mod2[-i] = (
                new_units_start_mod2[-i] - duration_exh_new[-i]
            )
        ...
        if len(new_units_end_mod2) > 0 and np.min(new_units_end_mod2) < 0:
            msg = "warning, negative value in exhumation time %0.2e" % np.min(
                new_units_end_mod2
            )
            print(msg)
    ...
    exhumation_df = pd.DataFrame(
        index=new_units_strat_list, columns=well_strat.columns, dtype="float64"
    )
    ...
    exhumation_df["deposition_code"] = np.array(deposition_codes, dtype=float)
    exhumation_df["eroded_thickness"] = np.array(
        new_units_thickness_list, dtype=float
    )
...
def find_hiatus(strat_units, age_start, age_end):
    ...
    for (
        ...
        age_end_bottom,
        age_start_top,
    ) in zip(strat_units[:-1], strat_units[1:], age_end[:-1], age_start[1:]):

        if age_end_bottom > age_start_top + 1e-4:
            print(
                "found hiatus between units %s (-%0.2f) "
                "and %s (%0.2f-)"
                % (
                    strat_unit_bottom,
                    age_end_bottom,
...
def calculate_initial_thickness(
    ...
    matrix_thickness = calculate_matrix_thickness(
        surface_porosity, compressibility, depth_top, depth_bottom
    )
    ...
    while initial_thickness_diff_max > max_decompaction_error:
        initial_thickness_new = decompact(
            matrix_thickness,
            surface_porosity,
            compressibility,
            initial_thickness,
        )
        initial_thickness_diff_max = np.max(
            np.abs(initial_thickness_new - initial_thickness)
        )
        initial_thickness = initial_thickness_new
        print(
            "max change in thickness over 1 decompaction iteration %0.2f"
...
def copy_df_columns(output_df, data_df, rows=None, ignore_age_columns=False):
    ...
    for row in rows:
        if row in data_df.index:
            ...
            msg = (
                "cannot find row item %s in data .csv file. "
...
def find_maximum_depth(input_df, exhumation_phases, max_decompaction_error):
    """
    ...
    for exhumation_phase in exhumation_phases:
        ...
        pre_exhumation_start = np.where(input_df.index == exhumation_phase)[0][
            0
        ]
        ...
        for i, strat_unit in enumerate(pre_exhumation_strat):
            ...
            if strat_unit == pre_exhumation_strat[0]:
                ...
                z_top = float(
                    input_df.loc[overlying_unit, "maximum_depth_bottom_temp"]
                )
        ...
        # deeply buried
        ind = (
            input_df["maximum_depth_top"] < input_df["maximum_depth_top_temp"]
        ) | input_df["maximum_depth_top"].isnull()
        input_df.loc[ind, "maximum_depth_top"] = input_df.loc[
            ind, "maximum_depth_top_temp"
        ]
        input_df.loc[ind, "maximum_depth_bottom"] = input_df.loc[
            ind, "maximum_depth_bottom_temp"
        ]

    return input_df


def construct_heat_flow_matrix(
    T, dz, dt, K, rho, c, fixed_upper_temperature, fixed_lower_temperature
):
    """
...
def construct_heat_flow_matrix_variable_z(
    T, z, dt, K, rho, c, fixed_upper_temperature, fixed_lower_temperature
):
    """
    set up matrix for finite difference eqs.
...
def create_heat_flow_vector_variable_z(
    ...
    if fixed_lower_temperature is not None:
        ...
        b[-1] = T[-1] + (Q[-1] + lower_bnd_flux / dz_bottom) * dt / (
            rho[-1] * c[-1]
        )
...
def solve_1D_heat_flow(
    ...
    try:
        ...
        raise ValueError(
            "error, solving matrix for temperature diffusion eq. failed"
        )
...
def solve_1D_diffusion(
    ...
    if A is None:
        A = construct_heat_flow_matrix_variable_z(
            C, z, dt, Ks, phi, c, fixed_upper_salinity, fixed_lower_salinity
        )
...
def get_geo_history(
    ...
    if np.min(exhumed_thicknesses) < min_exh_thickness:
        print(
            "warning, exhumed thicknesses of one or more phases does not "
    ...
    while thickness_ok is False and thickness_count < 100:
        ...
        if np.any(ind.values == True):
            print(
                "found unit with thickness < %0.1f m, adding %0.1f m"
            ...
            # Если ind_start - это список строковых индексов
            start_idx_position = well_strat.index.get_loc(ind_start[0])

            # Изменяем depth_bottom
            well_strat.iloc[
                start_idx_position:, well_strat.columns.get_loc("depth_bottom")
            ] += min_thickness

            # Изменяем depth_top (начиная со следующей строки)
            if start_idx_position + 1 < len(well_strat):
                well_strat.iloc[
                    start_idx_position + 1 :,
                    well_strat.columns.get_loc("depth_top"),
                ] += min_thickness
    ...
    well_strat["matrix_thickness"] = calculate_matrix_thickness(
    ...
    print("decompacting")
    well_strat["initial_thickness"] = calculate_initial_thickness(
        max_decompaction_error,
        well_strat["surface_porosity"],
        well_strat["compressibility"],
        well_strat["depth_top"],
    ...
    # add exhumation phases
    geohist_df = add_exhumation_phases(
        well_strat,
        exhumation_period_starts,
        exhumation_period_ends,
        exhumed_strat_units,
    ...
    geohist_df = copy_df_columns(
        geohist_df, strat_info_mod, rows=exhumed_units, ignore_age_columns=True
    )
    ...
    exhumation_phases = list(
        geohist_df.index[geohist_df["deposition_code"] == -1]
    )

    # find maximum burial depth of units:
    geohist_df = find_maximum_depth(
        geohist_df, exhumation_phases, max_decompaction_error
    )
    ...
    # present-day depth
    geohist_df.loc[ind, "matrix_thickness"] = calculate_matrix_thickness(
    ...
    # find hiatusses
    hiatus_list, hiatus_start_list, hiatus_end_list = find_hiatus(
        geohist_df.index[::-1],
    ...
    hiatus_df = pd.DataFrame(index=hiatus_list, columns=geohist_df.columns)
...
def reconstruct_strat_thickness(geohist_df, verbose=False):
    ...
    strat_thickness_df = pd.DataFrame(index=strat_units, columns=[])
    ...
    for timestep in geohist_df.index.values[::-1]:
        ...
        if geohist_df.loc[timestep, "deposition_code"] == 1:
            ...
            betas = [
                geohist_df.loc[s, "compressibility"] for s in strat_column
            ]
    ...
    if verbose is True:
        ...
        for s, th in zip(strat_column, thicknesses):
            ...
            print(
                s,
                geohist_df.loc[s, "present-day_thickness"],
                geohist_df.loc[s, "present-day_thickness_simulated"],
            )
...
def interpolate_param(
    param_start, param_end, nt_heatflow, fill_empty_value=True
):
    """
    ...
    int_array = np.resize(
        np.linspace(0, 1, nt_heatflow + 1)[1:], [len(param_start), nt_heatflow]
    ).T
    ...
    param_int = (
        int_array * (param_end_array - param_start_array) + param_start_array
    )

    return param_int


def interpolate_node_variables(
    formation_param_start, formation_param_end, n_timesteps
):
    """
    ...
    formation_var = interpolate_param(
        formation_param_start, formation_param_end, n_timesteps
    )
    ...
    node_var_bottom[:, :-1] = (
        formation_var[:, 1:] + formation_var[:, :-1]
    ) / 2.0
    ...
    node_var[:, 1::2] = node_var_mid
    node_var[:, 2::2] = node_var_bottom
    node_var[:, 0] = node_var_surface

    return node_var


def generate_thermal_histories(
    resample_t,
    n_nodes,
    time_array_bp,
    T_nodes,
    ...
    for nn in range(n_nodes):
        ...
        for p1, p2 in zip(prov_ages_start_n, prov_ages_end_n):

            if p1 <= burial_time[0]:
                print(
                    "warning, start deposition in basin earlier than "
                    "start provenance history"
                )
                print(
                    f"using a hard coded prov history of {provenance_start_temp} C to surface"
                )
            ...
            prov_time_fine = [
                np.linspace(
                    a, b, int(nt_prov / (len(provenance_times) - 1)) + 1
                )[:-1]
                for a, b in zip(provenance_times[:-1], provenance_times[1:])
            ]

            if len(prov_time_fine) <= 1:
                prov_T_cooling = np.linspace(
                    provenance_start_temp, burial_T[0], len(prov_time_fine[0])
                )
                prov_T = prov_T_cooling
            else:
                prov_T_depo = np.interp(
                    prov_time_fine[1],
                    Ts["age"].values,
                    Ts["surface_temperature"],
                )
                prov_T_cooling = np.linspace(
                    provenance_start_temp,
                    prov_T_depo[0],
    ...
    for nn in range(n_nodes):
        ...
        for p1, p2 in zip(prov_ages_start_n, prov_ages_end_n):
            ...
            if p1 <= burial_time[0]:
                ...
                print(
                    f"using a hard coded prov history start of 2 my before deposition to deposition age"
                )
                p1 = burial_time[0] + 2.0
                p2 = burial_time[0]
            ...
            if len(prov_time_fine) <= 1:
                prov_z_cooling = np.linspace(
                    5000.0, 0.0, len(prov_time_fine[0])
                )
...
def simulate_aft(
    ...
    aft_age_nodes = np.zeros((n_nodes, n_prov_scenarios, n_kinetic_scenarios))
    aft_ln_mean_nodes = np.zeros(
        (n_nodes, n_prov_scenarios, n_kinetic_scenarios)
    )
    aft_ln_std_nodes = np.zeros(
        (n_nodes, n_prov_scenarios, n_kinetic_scenarios)
    )
        ...
        for ng in range(n_grains):
            ...
            for n_prov in range(n_prov_scenarios):
                ...
                if log_tT_paths is True:
                    fn = os.path.join(
                        tT_path_filename,
                        f"tT_AHe_sample{nn}_grain{ng}_prov{n_prov}.txt",
                    )
                    print(
                        f"saving time-temp path for sample {nn}, grain {ng} provenance history {n_prov} to {fn}"
                    )
                    save_tT_path(t, T, fn)
                ...
                if mineral == "apatite":
                    ...
                    print(
                        "found a zircon grain, calculating AHe age using PyThermo ZRDAAM model"
                    )
                    ...
                    try:
                        ...
                        msg = (
                            "could not import pythermo, which is required for zircon AHe age calculation, "
                    ...
                    tT_in = np.array(
                        [t_Ma_young_to_old[::10], T_degC_young_to_old[::10]]
                    ).T  # time-temperature path as a 2D numpy array
                    ...
                    try:
                        ...
                        df_error = pd.DataFrame(
                            tT_in, columns=["time_Ma", "T_degC"]
                        )
                        error_fn = os.path.join(
                            tT_path_filename,
                            f"error_tT_AHe_sample{nn}_grain{ng}_prov{n_prov}.csv",
                        )
...
def calculate_viscosity_np(C, T):
    ...
    viscosity = (
        ...
        + (0.333 * C)
        + (1.65 + 91.9 * C**3)
        * np.exp(-(0.42 * (C**0.8 - 0.17) ** 2 + 0.045) * T**0.8)
    )
...
def run_burial_hist_model(
    ...
    :return:
    """
    year = 365.25 * 24.0 * 60 * 60.0

    # make sure exhumed thicknesses param is an array
    pybasin_params.exhumed_thicknesses = np.array(
    ...
    if save_csv_files is True:
        ...
        geohist_df.to_csv(
            os.path.join(
                output_dir,
                "geo_history_well_%s_ms%i.csv" % (well, model_scenario_number),
            ),
    ...
    active_fm = pd.DataFrame(
        index=strat_thickness_df.index,
        columns=strat_thickness_df.columns,
        dtype=bool,
    )

    for start_age, end_age in zip(
        strat_thickness_df.columns[:-1], strat_thickness_df.columns[1:]
    ):
        inactive_bool = np.logical_and(
            strat_thickness_df[start_age] == 0,
    ...
    hf_param_df = pd.DataFrame(
        index=burial_df.index,
        columns=[
            "K",
            "c",
            "HP",
    ...
    for ix in hf_param_df.index:
        ...
        hf_param_df.loc[ix, "matrix_thickness"] = geohist_df.loc[
            ix, "matrix_thickness"
        ]
        hf_param_df.loc[ix, "surface_porosity"] = geohist_df.loc[
            ix, "surface_porosity"
        ]

    # calculate evolution of porosity
    porosity_df = pd.DataFrame(
        index=burial_df.index, columns=burial_df.columns
    )
    k_df = pd.DataFrame(index=burial_df.index, columns=burial_df.columns)
    c_df = pd.DataFrame(index=burial_df.index, columns=burial_df.columns)
    rho_df = pd.DataFrame(index=burial_df.index, columns=burial_df.columns)
    hp_df = pd.DataFrame(index=burial_df.index, columns=burial_df.columns)
    ...
    for col in porosity_df.columns:
        for row in porosity_df.index:
            if active_fm.loc[row, col] == True and np.isnan(
                porosity_df.loc[row, col]
            ):
                porosity_df.loc[row, col] = hf_param_df.loc[
                    row, "surface_porosity"
                ]
    porosity_last = porosity_df[porosity_df.columns[-1]].dropna().values
    print(
        "final calculated porosity, mean=%0.2f, range=%0.2f-%0.2f"
    ...
    for ix in c_df.index:
        k_df.loc[ix] = (
            hf_param_df.loc[ix, "K"] ** (1.0 - porosity_df.loc[ix])
            * litho_props.loc["water", "thermal_conductivity"]
            ** porosity_df.loc[ix]
        )
        c_df.loc[ix] = (
            hf_param_df.loc[ix, "c"] * (1.0 - porosity_df.loc[ix])
            + litho_props.loc["water", "heat_capacity"] * porosity_df.loc[ix]
        )
        rho_df.loc[ix] = (
            hf_param_df.loc[ix, "rho_matrix"] * (1.0 - porosity_df.loc[ix])
            + porosity_df.loc[ix] * litho_props.loc["water", "density"]
        )
        hp_df.loc[ix] = hf_param_df.loc[ix, "HP"] * (1.0 - porosity_df.loc[ix])

    k_last = k_df[k_df.columns[-1]].dropna().values
    print(
        "final thermal conductivity, mean=%0.2f, range=%0.2f-%0.2f"
    ...
    nt_heatflows = np.array(
        [int(np.round(duration * 1e6 / dt)) for duration in durations]
    )
    ...
    if np.min(nt_heatflows) <= 0:
        msg = (
            "error, 0 heatflow timesteps for stratigraphic timestep %i of %i"
    ...
    top_fms = [
        np.where(active_fm[start_age] == True)[0][0]
    ...
    timestep = 0
    for start_age, end_age, nt_heatflow in zip(
        start_ages, end_ages, nt_heatflows
    ):
        active_fm_i = active_fm[start_age].values

        active_cells[timestep : (timestep + nt_heatflow), :-1:2] = active_fm_i
        active_cells[timestep : (timestep + nt_heatflow), 1::2] = active_fm_i

        active_nodes[timestep : (timestep + nt_heatflow), 1:-1:2] = active_fm_i
        active_nodes[timestep : (timestep + nt_heatflow), 2::2] = active_fm_i
    ...
    for start_age, end_age, nt_heatflow in zip(
        ...
        try:
            zs_bottom = interpolate_param(
                burial_df[start_age].values,
    ...
    for i in range(n_prov):
        prov_ages_start.append(geohist_df["provenance_age_start_%i" % (i + 1)])
        prov_ages_end.append(geohist_df["provenance_age_end_%i" % (i + 1)])
    ...
    active_prov = pd.isna(prov_ages_start_array[:, -1]) == False
    ...
    for start_age, end_age, nt_heatflow in zip(
        ...
        ks = interpolate_param(
            k_df[start_age].values.astype(float),
    ...
    # populate thermal parameter arrays:
    print(
        "setting grid node thermal params (rho, c, HP, phi), "
    ...
    for nti in range(nt_total):
        ...
        ind0 = np.where(np.diff(z_nodes[nti, active_nodes[nti]]) == 0)[0]
    ...
    surface_temp_array = np.interp(
        time_array_bp, Ts["age"].values * 1.0e6, Ts["surface_temperature"]
    )
    ...
    if pybasin_params.simulate_salinity is True:
        ...
        for i_stage, stage, marine_stage, nt_heatflow, dt_hf in zip(
            itertools.count(), stages, marine_stages, nt_heatflows, dt_hfs
        ):
            ...
            surface_salinity_array[timestep : (timestep + nt_heatflow)] = (
                salinity_stage
            )
        ...
        if surface_salinity_well is not None:
            for i in surface_salinity_well.index:
                ...
                ind_t = (
                    time_array_bp
                    <= surface_salinity_well.loc[i, "age_start"] * 1.0e6
                ) & (
                    time_array_bp
                    >= surface_salinity_well.loc[i, "age_end"] * 1.0e6
                )
                ...
                if True in ind_t:
                    ...
                    if (
                        "terrestrial"
                        in surface_salinity_well.loc[i, "surface_salinity"]
                    ):
                        ...
                        target_salinity = (
                            pybasin_params.salinity_seawater
                            + pybasin_params.salinity_seawater
                        ) / 2.0
                    else:
                        msg = (
                            "error, could not read surface salinity for well %s: "
                            % well
                        )
                        msg += str(
                            surface_salinity_well.loc[i, "surface_salinity"]
                        )
                        raise ValueError(msg)

                    print(
                        "updating surface salinity bnd, "
    ...
    for timestep in range(nt_total):
        ...
        T_nodes[timestep, active_nodes_i], A = solve_1D_heat_flow(
            T_init,
        ...
        if pybasin_params.simulate_salinity is True:
            ...
            if pybasin_params.constant_diffusivity is False:
                Dw = calculate_diffusion_coeff(
                    T_nodes[timestep, active_nodes_i], C_init
                )
            else:
            ...
            Ks_nodes = (
                ...
                / tortuosity_nodes[timestep, active_nodes_i]
                * Dw
            )

            Ks_cells = (Ks_nodes[1:] + Ks_nodes[:-1]) / 2.0

            C_nodes[timestep, active_nodes_i], A_s = solve_1D_diffusion(
                C_init,
                z_nodes[timestep, active_nodes_i],
                dt_hf * year,
                Ks_cells,
            ...
            P = z_nodes[timestep, active_nodes_i] * (
                ...
                + ((1.0 - porosity_nodes[timestep, active_nodes_i]) * 2650.0)
            )
            density = equations_of_state_batzle1992(
                P,
                T_nodes[timestep, active_nodes_i],
                C_nodes[timestep, active_nodes_i],
            )

            # calculate solute flux at top node
            dC_top = (
                C_nodes[timestep, active_nodes_i][1]
                - C_nodes[timestep, active_nodes_i][0]
            )
            dx_top = (
                z_nodes[timestep, active_nodes_i][1]
            ...
            q_solute_bottom[timestep] = (
                density[-1] * Ks_cells[-1] * dC_bottom / dx_bottom
            )

        if np.any(np.isnan(T_nodes[timestep, active_nodes_i])):

            for cs, ac, rho in zip(
                cell_strat, active_cells[timestep], rho_nodes[timestep]
            ):
                print(cs, ac, rho)
        ...
        if timestep in cumulative_steps or timestep == nt_total - 1:
            ...
            print(
                "step %i, %0.2f Ma, n nodes = %i, max z = %0.1f, T = %0.1f - %0.1f"
                % (
                    timestep,
